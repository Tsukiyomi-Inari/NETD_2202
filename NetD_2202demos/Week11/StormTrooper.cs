...
namespace W8_Collections
{
    class StormTrooper
    {
        ...
        public static void FileWrite(List<StormTrooper> inputTroopers, String fileName)
        {
            FileStream fsWrite = new FileStream(fileName, FileMode.Create, FileAccess.Write);
            StreamWriter sWriter = new StreamWriter(fsWrite);
            sWriter.Write(FileCreateCSV(inputTroopers));
            sWriter.Close();
            fsWrite.Close();
        }

        private static String FileCreateCSV(List<StormTrooper> inputTroopers)
        {
